...
import { ShoppingCart, Truck, RotateCcw, CreditCard, User, Star, Mail, Phone, MessageCircle, FileText, HelpCircle } from 'lucide-react';
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {helpCategories.map((category, index) => (
              ...
              >
                <div className={`w-16 h-16 bg-gradient-to-br ${category.color} rounded-2xl flex items-center justify-center mb-6 mx-auto group-hover:scale-110 group-hover:rotate-6 transition-transform duration-300`}>
                ...
                <div className="mt-4 text-center text-primary-600 font-semibold group-hover:text-primary-700">
                  En savoir plus →
                </div>
              </Link>
            ))}
            ...
            {contactMethods.map((method, index) => (
              ...
              >
                <div className={`w-14 h-14 bg-gradient-to-br ${
                  method.color === 'blue' ? 'from-primary-500 to-primary-600' :
                  method.color === 'purple' ? 'from-secondary-500 to-secondary-600' :
      ...
      <section className="py-16 relative z-10">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="bg-white/90 backdrop-blur-sm rounded-3xl shadow-2xl p-8 md:p-12 border border-gray-200/50">
            <h2 className="text-3xl font-bold bg-gradient-to-r from-primary-600 to-primary-700 bg-clip-text text-transparent mb-6 text-center">
              Liens Utiles
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {quickLinks.map((link, index) => (
                <Link
                  key={index}
                  to={link.link}
                  className="flex items-center p-4 bg-gradient-to-r from-primary-50 to-secondary-50 rounded-xl hover:from-primary-100 hover:to-secondary-100 transition-all duration-300 group"
                >
                  <span className="mr-3 text-xl group-hover:scale-110 transition-transform">→</span>
                  <span className="text-gray-700 font-medium group-hover:text-primary-600">{link.text}</span>
                </Link>
              ))}
            </div>
          </div>
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <div className="bg-gradient-to-r from-primary-600 to-primary-700 rounded-3xl shadow-2xl p-8 md:p-12 text-white">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              ...
              >
                <FileText className="w-5 h-5 inline mr-1" /> Voir la FAQ
              </Link>
              <Link 
                to="/contact"
